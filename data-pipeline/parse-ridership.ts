/**
 * OA-12921 (역별 일별 시간대별 승하차인원, CP949)에서 1~8호선 하차 인원을
 * (호선, 역번호) 단위로 집계한다. 역번호가 노선 순서를 담고 있어
 * 역 목록 생성의 근거로도 쓴다.
 * CSV 버킷: 06시이전, 06-07시간대, ..., 23-24시간대, 24시이후 (20개)
 * → alightByHour 인덱스 0(=05시 버킷, 06시 이전 포함) ~ 19(=24시 이후)
 */
...
export function parseRidership(csvPath: string = RIDERSHIP_CSV): RidershipResult {
  ...
  const sums = new Map<string, Record<DayType, Float64Array>>();
  ...
  for (const row of rows.slice(1)) {
    if (row[cKind] !== "하차" || !/^[1-8]호선$/.test(row[cLine])) continue;
    const key = `${row[cLine]}|${row[cCode]}`;

    let sum = sums.get(key);
    if (!sum) {
      sum = { weekday: new Float64Array(BUCKET_COUNT), weekend: new Float64Array(BUCKET_COUNT) };
      sums.set(key, sum);
    ...
    for (let b = 0; b < BUCKET_COUNT; b++) {
      sum[dayType][b] += Number(row[cFirstBucket + b]) || 0;
    }
  ...
  for (const [key, sum] of sums) {
    ...
    const avg = (dayType: DayType) => {
      const n = days.get(key)![dayType].size;
      return Array.from(sum[dayType], (v) => (n > 0 ? Math.round((v / n) * 10) / 10 : 0));
    };
    byLineCode.set(key, {
      csvLine,
      code,
      names: [...names.get(key)!],
      alightByHour: { weekday: avg("weekday"), weekend: avg("weekend") },
    });
  }
