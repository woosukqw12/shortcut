...
await page.waitForSelector("text=사실상 동점이에요");
console.log("직행:", (await page.textContent("section >> text=앉을 확률"))?.trim().replace(/\s+/g, " "));
console.log("동점 대안:", (await page.textContent("text=사실상 동점이에요"))?.trim().replace(/\s+/g, " "));
...
await page.waitForSelector("text=도보 약 2분 32초");
await page.waitForSelector("text=환승 통로와 가까운 문은 1-1문");
await page.waitForSelector("text=통로에서 나오면 3-2문 앞");
